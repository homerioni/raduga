...
import { ModalProductContent } from '../../../../components/admin/modals/ModalProductContent';
...
export default function AdminProductsPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => getProducts({ qty: 50, search: searchDebounce || undefined, page }),
  });
  ...
  const onDel = () =>
    modals.openConfirmModal({
      ...
      centered: true,
      children: <Text>{selectedItems.map((product) => product.name).join(', ')}</Text>,
      labels: { confirm: 'Да, удалить', cancel: 'Отменить' },
      confirmProps: { color: 'red' },
      onConfirm: () => {
        deleteProducts(selectedItems.map((product) => product.id)).then(() => refetch());
        setSelectedItems([]);
      },
    });
  ...
  const onEdit = () => {
    modals.open({
      ...
      size: 'xl',
      children: <ModalProductContent data={selectedItems[0]} refetch={refetch} />,
    });
    setSelectedItems([]);
  };
  ...
  return (
    <Stack gap={10}>
      ...
      {data?.totalPages && data?.totalPages > 1 && (
        <Center>
          <Pagination total={data.totalPages} value={page} onChange={(num) => setPage(num)} />
        </Center>
      )}
    </Stack>
  );
